...
public class GUI extends Application {
	...
	private void preparePrimaryStage(Stage primaryStage) {
		// Set the stage
		primaryStage.setTitle("FXTyping");
			...
			public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
				if (newValue.trim().equals(sentences[sentenceIndex])) {
					if (++sentenceIndex >= sentences.length) {
						...
						progressBar.setProgress((double) sentenceIndex / sentences.length);
						return;
					}
					...
					txtfTyping.setStyle("-fx-text-fill: black;");
					errors = 0;
		...
		progressBar.setStyle("-fx-accent: rgb(0, 125, 0);");
		...
		column1.getChildren().addAll(lblSentence, txtfSentence, txtfTyping, progressBar, buttonsRow);
